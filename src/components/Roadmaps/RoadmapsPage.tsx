...
const allGroups = [
  {
    group: 'Role Based Learning',
    roadmaps: roleRoadmaps,
  },
  {
    group: 'Skill Based Learning',
...
export function RoadmapsPage() {
  ...
  return (
    <div style={{ backgroundColor: '#66fcf1' }} className="flex flex-col items-start sm:items-center">
      <button
        onClick={() => {
      ...
      <div
  className={cn(
    'hidden w-full flex-col sm:w-[180px] sm:border-r sm:bg-gradient-to-l sm:pt-6',
    {
        ...
        <div className="flex flex-grow flex-col gap-6 pb-20 pt-2 sm:pt-8">
          {visibleGroups.map((group) => (
            <div key={`${group.group}-${group.roadmaps.length}`}>
              <h2 className="mb-2 text-xs uppercase tracking-wide text-black">
                {group.group}
              </h2>
              ...
              <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2 md:grid-cols-3">
                {group.roadmaps.map((roadmap) => (
                  <a
                    key={roadmap.link}
                    className="rounded-md border bg-white px-3 py-2 text-left text-sm shadow-sm transition-all hover:border-black hover:bg-gray-50"
                    href={roadmap.link}
                  >
                    {roadmap.title}
                  </a>
